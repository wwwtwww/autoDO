...
* **定时唤醒与解锁**：
  ...
  * 唤醒后通过 `PowerManager.WakeLock` 强制亮屏，并在备用机处于“免密/仅滑动解锁”状态下利用系统 `KeyguardManager.requestDismissKeyguard()` 接口安全解除锁屏。
* **打卡周期与例外避让**：
  * 支持自定义基础打卡周期（如勾选周一至周五）。
  * 支持配置“例外日期”（如添加某天为强制打卡补班日，或某天为强制休息节假日）。
  * 每天调度打卡任务前，优先读取本地例外列表，再比对基础周期，决定当天是否执行打卡，全程无网络依赖。
* **进程保活与权限引导**：
  * **无障碍系统级保活**：基于 Android 原生的 `AccessibilityService`，系统默认赋予极高的进程优先级，配合权限设置实现免后台通知的静默强力保活。
